...
<body>
    ...
    <div class="border">
        ...
        {sdcms:rs pagesize="15" field="bbsid,uface,uname,createdate,sd_bbs_reply.islock,replyid,content,reply" table="sd_bbs_reply" join="left join sd_user on sd_bbs_reply.userid=sd_user.id" where="istopic=0 $where" order="replyid desc" key="replyid"}
                ...
                <div class="ui-media-body">
                    <div class="ui-media-header">{$rs[uname]} <span class="ui-font-14 ui-text-gray ui-ml-15">发表于：{date('Y-m-d H:i:s',$rs[createdate])} {if $rs[islock]==0}<span class="ui-text-red">【未审】</span>{/if}</span>
                            <div class="ui-fr ui-font-14">
                                ...
                                <a href="{U('edit','id='.$rs[replyid].'')}" class="ui-mr"><i class="ui-icon-edit text-gray"></i> 编辑</a>
