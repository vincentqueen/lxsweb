            ...
            <div class="ui-tabs-content">
                <div class="ui-tabs-pane active">
                    ...
                    <div class="ui-form-group ui-row" id="domain">
                        ...
                        <div class="col-right">
                            <input type="text" name="t14" class="ui-form-ip" ><span class="input-tips">例：news.baidu.com{if $isbiz==0}，域名未授权，本功能无法使用。{/if}</span>
                        </div>
                    </div>
                    ...
                    <div class="ui-form-group ui-row{if C('user_open')==2} dis{/if}">
                        ...
                        <div class="col-right col-right-top">
                            ...
                            <label class="ui-checkbox"><input type="checkbox" name="t15[]" id="t15_{$rs[gid]}" value="{$rs[gid]}"><i></i>{$rs[gname]}</label>
                            {/sdcms:rs}
                        </div>
                    </div>
                    <!--1111-->
                ...
                <div class="ui-tabs-pane">
                    <!--4444-->
                    {foreach $field as $rs}
                    <div class="ui-form-group ui-row{if $rs['field_type']==7} dis{/if}">
                        ...
                        <div class="{if in_array($rs['field_type'],[12,13,15])}col-right-full{else}col-right{/if}{if in_array($rs['field_type'],[9,10])} col-form-label{/if}">
                            {switch $rs['field_type']}
                                {case 1}<input type="text" name="{$rs['field_key']}" id="{$rs['field_key']}" class="ui-form-ip"{if $rs['field_length']!=0} maxlength="{$rs['field_length']}"{/if} value="{deal_default($rs['field_default'])}" {deal_rule($rs['field_rule'],$rs['field_title'])}>{/case}
                                {case 2}<input type="text" name="{$rs['field_key']}" id="{$rs['field_key']}" class="ui-form-ip datepick"{if $rs['field_length']!=0} maxlength="{$rs['field_length']}"{/if} value="{deal_default($rs['field_default'])}" readonly {deal_rule($rs['field_rule'],$rs['field_title'])}>{/case}
                                ...
                                {/case}
                                {case 6}<input type="password" name="{$rs['field_key']}" id="{$rs['field_key']}" class="ui-form-ip" value="{deal_default($rs['field_default'])}" {deal_rule($rs['field_rule'],$rs['field_title'])}>{/case}
                                ...
                                {php $data=explode("|",$key)}
                                    ...
                                    <label class="ui-radio"><input type="radio" name="{$rs['field_key']}" value="{$data[1]}" {deal_rule($rs['field_rule'],$rs['field_title'])} {if $rs['field_default']=="".$data[1].""} checked{/if}>
                                    <i></i>{$data[0]}</label>
                                ...
                                {foreach $arr as $j=>$key}
                                {php $data=explode("|",$key)}
                                <label class="ui-checkbox"><input type="checkbox" name="{$rs['field_key']}[]" value="{$data[1]}" {deal_rule($rs['field_rule'],$rs['field_title'])} {if stristr(",".$rs['field_default'].",",",".$data[1].",")} checked{/if}><i></i>{$data[0]}</label>
                                {/foreach}
                                {/case}
                                {case 11}
