...
<body>
    <div class="position">当前位置：栏目管理 > <a href="{U('extend/index')}">内容扩展</a> > <a href="{THIS_LOCAL}">{$mtitle}</a> > 字段管理</div>
    <div class="border">
        <!---->
        <a href="{U('add',"eid=".$eid."")}" class="ui-btn ui-btn-info">添加字段</a>
        ...
        <table class="ui-table ui-table-border ui-table-hover ui-table-striped ui-mt ui-mb">
            ...
            {/rs:eof}
            <tr>
                <td><input type="hidden" name="eid[]" value="{$rs[id]}"><input type="text" name="ordnum[]" class="ui-form-ip" id="ordnum_{$rs[id]}" value="{$rs[ordnum]}" data-rule="required;int;"></td>
                <td>{$rs[id]}</td>
