...
<body>
    <div class="position">当前位置：栏目管理 > <a href="{U('form/index')}">表单管理</a> > <a href="{THIS_LOCAL}">{$ftitle}</a> > 字段管理</div>
    <div class="border">
        <!---->
        <a href="{U('add',"fid=".$fid."")}" class="ui-btn ui-btn-info ui-mr-sm">添加字段</a>
        ...
        <table class="ui-table ui-table-border ui-table-hover ui-table-striped ui-mt ui-mb">
            ...
            <tr>
                ...
                {case 14}数据集{/case}
                {/switch}</td>
                <td><label class="ui-switch ui-switch-info"><input type="checkbox" {if $rs[islist]==1} checked{/if} data-url="{U('switchs','type=1&id='.$rs[id].'')}"><span class="ui-switch-checkbox ui-switch-text"></span></label></td>
