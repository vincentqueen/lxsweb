...
<body>
    <div class="position">当前位置：栏目管理 > <a href="{U('model/index')}">模型管理</a> > <a href="{THIS_LOCAL}">{$mtitle}</a> > 字段管理</div>
    <div class="border">
        <!---->
        <a href="{U('add',"mid=".$mid."")}" class="ui-btn ui-btn-info">添加字段</a>
        ...
        <table class="ui-table ui-table-border ui-table-hover ui-table-striped ui-mt ui-mb">
            ...
            <tbody>
            {sdcms:rs top="0" table="sd_model_field" where="model_id=$mid" order="ordnum,id"}
            {rs:eof}<tr>
            ...
            <tr>
                ...
                {case 15}下载集{/case}
                {/switch}</td>
                <td>{iif($rs[issys]==1,'系统字段','<em>用户字段</em>')}</td>
