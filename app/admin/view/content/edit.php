            ...
            <div class="ui-tabs-content">
                ...
                <div class="ui-tabs-pane{if $index==0} active{/if}">
                            ...
                            {foreach $cate as $jc=>$rs}
                                {if get_admin_info('pid')!=0}
                                    ...
                                    {if in_array($jc,$lever)}
                                         <option value="{$rs['cateid']}"{if $rs['catetype']<0 || strpos($rs['sonid'],',') || $rs['cateid']==$classid || $rs['catetype']!=$model_id} disabled{/if}{if in_array($rs['cateid'],explode(",",trim($record['subid'],",")))} selected{/if}>{str_repeat("　",$rs['depth'])}{$rs['catename']}</option>
                                    {/if}
                                {else}
                                     <option value="{$rs['cateid']}"{if $rs['catetype']<0 || strpos($rs['sonid'],',') || $rs['cateid']==$classid || $rs['catetype']!=$model_id} disabled{/if}{if in_array($rs['cateid'],explode(",",trim($record['subid'],",")))} selected{/if}>{str_repeat("　",$rs['depth'])}{$rs['catename']}</option>
                                {/if}
                    ...
                    <div class="ui-form-group ui-row">
                        ...
                        <div class="col-right col-right-top">
                            <label class="ui-radio"><input type="radio" name="isauto" id="isauto_0" value="0"{if $record['isauto']==0} checked{/if}><i></i>否</label>
                            <label class="ui-radio"><input type="radio" name="isauto" id="isauto_1" value="1"{if $record['isauto']==1} checked{/if}><i></i>是</label>                                                        							
                        </div>
                    </div>
                    {/if}
                    ...
                    <div class="ui-form-group ui-row{if C('user_open')==2 || $leverstate==0} dis{/if}">
                        ...
                        <div class="col-right col-right-top">
                            ...
                            <label class="ui-checkbox"><input type="checkbox" name="view_lever[]" value="{$rg[gid]}"{if in_array($rg[gid],explode(',',$record['view_groupid']))} checked{/if}><i></i>{$rg[gname]}</label>
                            {/sdcms:rg}
                        </div>
                    </div>
                    {/if}
                    ...
                    <div class="ui-form-group ui-row"{if $rs['field_type']==7} style="display:none;"{/if}>
                        ...
                        <div class="{if in_array($rs['field_type'],[12,13,15])}col-right-full{else}col-right{/if}{if $rs['field_type']==9} col-right-top{/if}">
                            ...
                            <input type="text" name="{$rs['field_key']}" id="{$rs['field_key']}" class="ui-form-ip{if in_array($rs['field_key'],['url','tags','showskin'])} radius-right-none{/if}"{if $rs['field_length']!=0} maxlength="{$rs['field_length']}"{/if} value="{$record[$rs['field_key']]}" {deal_rule($rs['field_rule'],$rs['field_title'])}>
                            {if $rs['field_key']=='url'}
                            ...
                            {case 2}<input type="text" name="{$rs['field_key']}" id="{$rs['field_key']}" class="ui-form-ip{if $rs['field_key']!=='createdate'} datepick{else} datepick-time{/if}" {if $rs['field_length']!=0} maxlength="{$rs['field_length']}"{/if} value="{if $rs['field_key']!=='createdate'}{date('Y-m-d',$record[$rs['field_key']])}{else}{date('Y-m-d H:i:s',$record[$rs['field_key']])}{/if}" {deal_rule($rs['field_rule'],$rs['field_title'])}>{/case}
                            ...
                            {/foreach}
                            {if $rs['field_key']=='islock' && $record[$rs['field_key']]<0}
                            <label class="ui-radio"><input type="radio" name="islock" value="-1"{if $record[$rs['field_key']]=="-1"} checked{/if}><i></i>回收站</label>
                            {/if}
                            {/case}
                            {case 10}
                            <div class="ui-pt"></div>
                            ...
                            <table class="ui-table ui-table-border ui-mt ui-w-auto">
                                ...
                                <tbody id="downlist_{$rs['field_key']}">
                                    ...
                                    <tr num="{$num}">
                                        ...
                                        <td>
                                            <a href="javascript:;" class="down-prev mr-sm"><i class="ui-icon-up"></i>上移</a>
                                            <a href="javascript:;" class="down-next mr-sm"><i class="ui-icon-down"></i>下移</a>
                ...
                <div class="ui-tabs-pane">
					...
					<div class="ui-form-group ui-row">
                        ...
                        <div class="col-right">
							{switch $rs['field_type']}
                            {case 1}<input type="text" name="extend[{$rs['field_key']}]" id="{$rs['field_key']}" class="ui-form-ip" value="{if isset($extend[$rs['field_key']])}{$extend[$rs['field_key']]}{/if}" >{/case}
                            {case 2}
