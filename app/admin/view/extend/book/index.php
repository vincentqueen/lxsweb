...
<body>
    <div class="position">当前位置：扩展管理 > <a href="{U('index')}">留言管理</a></div>
    <div class="border">
        <!---->
        <div class="navbar">
            <div class="lefter">
        ...
        <table class="ui-table ui-table-border ui-table-hover ui-table-striped ui-mb">
            ...
            <tbody>
            {sdcms:rs pagesize="20" table="sd_book" where="$where" order="ontop desc,id desc"}
            {rs:eof}
            <tr>
            ...
            <tr>
                ...
                <td>{$rs[id]}</td>
                <td class="ui-text-left">{$rs[truename]}</td>
                <td>{$rs[tel]}</td>
                <td>{$rs[mobile]}</td>
                <td>{date('Y-m-d H:i:s',$rs[createdate])}</td>
                <td>{iif(strlen($rs[reply])!=0,'已回复','<em>未回复</em>')}</td>
