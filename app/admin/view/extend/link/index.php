...
<body>
    ...
    <div class="border">
        ...
        <div class="navbar">
            <div class="lefter">
                <a href="javascript:;" data-url="{U('add')}" class="add-iframe ui-btn ui-btn-info ui-mr-sm">添加链接</a>
                <a href="javascript:;" class="ui-btn ui-btn-info ui-dropdown-show ui-mr-sm" data-target="#dropdown-1">批量操作</a>
                <a href="{U('config')}" class="ui-btn ui-btn-yellow ui-mr-sm">链接配置</a>
                ...
                <span class="ui-btn-group ui-btn-group-yellow ui-btn-group-bg">
                    ...
                    <a class="ui-btn-group-item{if $type==3} active{/if}" href="{U('index','type=3')}">文字</a>
                    <a class="ui-btn-group-item{if $type==4} active{/if}" href="{U('index','type=4')}">Logo</a>
                </span>
            </div>            
        </div>
        ...
        <table class="ui-table ui-table-border ui-table-hover ui-table-striped ui-mb ui-mt">
            <thead class="ui-thead-gray">
                <tr>
                    ...
                    <th>网站名称</th>
                    <th width="300">网址</th>
                    {if C('link_class')==1}<th width="130">分类</th>{/if}
                    <th width="80">状态</th>
                    <th width="150">操作</th>
                </tr>
            </thead>
            ...
            {rs:eof}
            <tr>
                <td colspan="{if C('link_class')==0}7{else}8{/if}">暂无资料</td>
            </tr>
            {/rs:eof}
            <tr>
                ...
                <td>{$rs[id]}</td>
                <td class="ui-text-left">{$rs[webname]}</td>
                <td class="ui-text-left"><a href="{$rs[weburl]}" target="_blank">{$rs[weburl]}</a></td>
                {if C('link_class')==1}<td>{foreach $arr as $key=>$val}{php list($a,$b)=explode('|',$val)}{if $b==$rs[classid]}{$a}{/if}{/foreach}</td>{/if}
