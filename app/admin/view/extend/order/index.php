    ...
    <div class="border">
        ...
        <div class="navbar">
            <div class="lefter">
                ...
                <span class="ui-btn-group ui-btn-group-yellow ui-btn-group-bg">
                    ...
                    <a class="ui-btn-group-item{if $type==1} active{/if}" href="{U('index','type=1')}">未付款</a>
                    <a class="ui-btn-group-item{if $type==2} active{/if}" href="{U('index','type=2')}">已付款</a>
                    <a class="ui-btn-group-item{if $type==3} active{/if}" href="{U('index','type=3')}">未处理</a>
                    <a class="ui-btn-group-item{if $type==4} active{/if}" href="{U('index','type=4')}">已处理</a>
                </span>
            </div>
        ...
        <table class="ui-table ui-table-border ui-table-hover ui-table-striped ui-mb">
            ...
            <tr>
                ...
                <td class="ui-text-left ui-font-14">{$rs[orderid]}</td>
                <td class="ui-text-left ui-font-14">{$rs[pro_name]}</td>
                <td>{$rs[pro_num]}</td>
                <td>{$rs[pro_price]}</td>
                <td>{$rs[truename]}</td>
                <td>{$rs[mobile]}</td>
                ...
                <td><label class="ui-switch ui-switch-info"><input type="checkbox" {if $rs[isover]==1} checked{/if} data-url="{U('switchs','id='.$rs[id].'')}"><span class="ui-switch-checkbox ui-switch-text"></span></label></td>
                <td><a href="javascript:;" data-url="{U('edit',"id=".$rs[id]."")}" class="edit-iframe">查看</a>　<a href="javascript:;" class="del" data-url="{U('del','id='.$rs[id].'')}">删除</a></td>
            </tr>
            {/sdcms:rs}
            </tbody>
        </table>
