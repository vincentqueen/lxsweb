...
<body>
    ...
    <div class="home">
        ...
        <div class="ui-row ui-mt-20 homeother">
            ...
            <div class="ui-col-4">
            	<!---->
                <div class="box">
                    <div class="subject"><a href="{U('content/index')}" target="_blank">更多>></a>最新文章</div>
                    <ul class="ui-list">
                        {sdcms:rs top="4" table="sd_content" order="id desc"}
						<li style="line-height: 30px;"><a href="{$rs[link]}" title="{cutstr($rs[title],80,1)}" target="_blank">{cutstr($rs[title],80,1)}</a></li>
						{/sdcms:rs}
                    </ul>
                 </div> 
                <!---->
            	...
            	<!---->                 
                 <div class="box">   
                    ...
                    <div class="log">
                        ...
                        <p><strong><span>{date('Y-m-d H:i',$rs[logindate])}</span>{$rs[loginname]}</strong>{enhtml($rs[loginip])}<span>{$rs[loginmsg]}</span></p>
                        {/sdcms:rs}
                    </div>
                </div>
                <!---->
    ...
    <div class="copyright"><span>{if false}当前版本：{C('version')} {C('version_number')}{if get_admin_info('pid')==0}　<a href="javascript:;" class="checkupdate ui-btn ui-btn-sm ui-btn-blue ui-btn-outline-blue">检查更新</a>{/if}　数据库：{$dbtype}　授权类型：{$authtype}{/if}</span>Powered By <a href="http://www.ewinkee.cn" target="_blank">时代赢客</a> @ {date('Y')} Inc.</div>
    <script>
	$(function()
	{
