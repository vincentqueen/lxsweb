...
<body>
    ...
    <div class="border">
        ...
        <div class="navbar">
            <div class="lefter">
                <a href="javascript:;" class="btach ui-btn ui-btn-info ui-mr-sm">批量删除</a>
        ...
        <table class="ui-table ui-table-border ui-table-hover ui-table-striped ui-mb ui-mt">
            ...
            <tbody>
            {sdcms:rs pagesize="20" table="sd_admin_login_log" where="$where" order="id desc"}
            {rs:eof}
            <tr>
            ...
            <tr>
                ...
                <td>{$rs[id]}</td>
                <td>{$rs[loginname]}</td>
                <td>{enhtml($rs[loginip])}</td>
                <td class="ui-text-left">{$rs[loginmsg]}</td>
                <td>{date('Y-m-d H:i:s',$rs[logindate])}</td>
