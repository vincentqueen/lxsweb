...
<body>
    ...
    <div class="border">
        ...
        <a href="javascript:;" data-url="{U('add','fid='.$fid.'')}" class="{if $fid==0}add-iframe{else}add-iframe-son{/if} ui-btn ui-btn-info ui-mr-sm">添加菜单</a>
        {if $fid>0}
        <a href="{U('index')}" class="ui-btn ui-btn-yellow">返回上级</a>
        {/if}
        ...
        <table class="ui-table ui-table-border ui-table-hover ui-table-striped ui-mb ui-mt">
            ...
            <tbody>
            {sdcms:rp top="0" table="sd_admin_menu" where="followid=$fid" order="ordnum,id"}
            {php $classid=$rp[id]}
            <tr>
                ...
                <td><label class="ui-switch ui-switch-info"><input type="checkbox" {if $rp[islock]==1} checked{/if} data-url="{U('switchs','id='.$rp[id].'')}"><span class="ui-switch-checkbox ui-switch-text"></span></label></td>
