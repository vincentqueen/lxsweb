...
<body>
    <div class="position">当前位置：栏目管理 > <a href="<?php echo THIS_LOCAL;?>">内容扩展</a></div>
    <div class="border">
        <!---->
        <a href="javascript:;" data-url="<?php echo U('add');?>" class="add-iframe ui-btn ui-btn-info">添加方案</a>
        ...
        <table class="ui-table ui-table-border ui-table-hover ui-table-striped ui-mt ui-mb">
            ...
            <tbody>
            <?php $array_rs=$this->db->load("select * from sd_extend  where 1=1  order by ordnum,id ",0,false,0);$total_rs=count($array_rs);if($total_rs==0){ ?><tr>
...
<?php $i=0;} foreach($array_rs as $rs){ $i++;?>
            ...
            <tr>
                <td><input type="hidden" name="mid[]" value="<?php echo $rs['id'];?>"><input type="text" name="ordnum[]" class="ui-form-ip" id="ordnum_<?php echo $rs['id'];?>" value="<?php echo $rs['ordnum'];?>" data-rule="required;int;"></td>
                ...
                <td><a href="<?php echo U('extendfield/index',"eid=".$rs['id']."");?>"><span class="ui-icon-file-text"></span> 字段管理</a>　<a href="javascript:;" data-url="<?php echo U('edit',"id=".$rs['id']."");?>" class="edit-iframe"><span class="ui-icon-edit"></span> 编辑</a>　<a href="javascript:;" class="del" data-url="<?php echo U('del','id='.$rs['id'].'');?>"><span class="ui-icon-delete"></span> 删除</a></td>
            </tr>
            <?php } if($total_rs>0){ ?>
<?php }?>
            </tbody>
