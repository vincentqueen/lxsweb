...
<body>
    <div class="position">当前位置：栏目管理 > <a href="<?php echo THIS_LOCAL;?>">表单管理</a></div>
    <div class="border">
        <!---->
        <a href="<?php echo U('add');?>" class="ui-btn ui-btn-info">添加表单</a>
        ...
        <table class="ui-table ui-table-border ui-table-hover ui-table-striped ui-mt ui-mb">
            ...
            <tbody>
            <?php $array_rs=$this->db->load("select * from sd_form  where 1=1  order by ordnum,id ",0,false,0);$total_rs=count($array_rs);if($total_rs==0){ ?>
            <tr>
...
<?php $i=0;} foreach($array_rs as $rs){ $i++;?>
            ...
            <tr>
                ...
                <td class="ui-text-left"><?php echo $rs['title'];?></td>
                <td><?php echo $rs['tablename'];?></td>
                <td><label class="ui-switch ui-switch-info"><input type="checkbox" <?php if ($rs['isuser']==1) { ?> checked<?php }?> data-url="<?php echo U('switchs','type=1&id='.$rs['id'].'');?>"><span class="ui-switch-checkbox ui-switch-text"></span></label></td>
                ...
                <td><a href="<?php echo U('home/form/index',"fid=".$rs['id']."");?>" target="_blank"><span class="ui-icon-share"></span> 前台列表</a>　<a href="<?php echo U('home/form/add',"fid=".$rs['id']."");?>" target="_blank"><span class="ui-icon-edit"></span> 前台提交</a></td>
                <td><a href="<?php echo U('formdata/index',"fid=".$rs['id']."");?>"><span class="ui-icon-database"></span> 数据管理</a>　<a href="<?php echo U('formfield/index',"fid=".$rs['id']."");?>"><span class="ui-icon-file-text"></span> 字段管理</a>　<a href="<?php echo U('edit',"id=".$rs['id']."");?>"><span class="ui-icon-edit"></span> 编辑</a>　<a href="javascript:;" class="del" data-url="<?php echo U('del','id='.$rs['id'].'');?>"><span class="ui-icon-delete"></span> 删除</a></td>
            </tr>
            <?php } if($total_rs>0){ ?>
<?php }?>
            </tbody>
