...
<body>
    <div class="position">当前位置：栏目管理 > <a href="<?php echo THIS_LOCAL;?>">模型管理</a></div>
    <div class="border">
        <!---->
        <a href="<?php echo U('add');?>" class="ui-btn ui-btn-info">添加模型</a>
        <a href="javascript:;" data-url="<?php echo U('copy');?>" class="copy-iframe ui-btn ui-btn-yellow">复制模型</a>
        ...
        <table class="ui-table ui-table-border ui-table-hover ui-table-striped ui-mt ui-mb">
            ...
            <tbody>
            <?php $array_rs=$this->db->load("select * from sd_model  where 1=1  order by ordnum,id ",0,false,0);$total_rs=count($array_rs);if($total_rs==0){ ?>
<?php } else{ ?>
<?php $i=0;} foreach($array_rs as $rs){ $i++;?>
            <tr>
                ...
                <td><a href="<?php echo U('modelfield/index',"mid=".$rs['id']."");?>"><span class="ui-icon-file-text"></span> 字段管理</a>　<a href="javascript:;" class="copy-iframe" data-url="<?php echo U('copy',"id=".$rs['id']."");?>"><span class="ui-icon-file-copy"></span> 复制</a>　<a href="<?php echo U('edit',"id=".$rs['id']."");?>"><span class="ui-icon-edit"></span> 编辑</a>　<a href="javascript:;" class="del" data-url="<?php echo U('del','id='.$rs['id'].'');?>"><span class="ui-icon-delete"></span> 删除</a></td>
            </tr>
            <?php } if($total_rs>0){ ?>
<?php }?>
            </tbody>
