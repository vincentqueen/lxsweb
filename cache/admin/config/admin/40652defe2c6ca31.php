...
<div class="border">
    <!---->
    <a href="javascript:;" data-url="<?php echo U('add');?>" class="add-iframe ui-btn ui-btn-info">添加用户</a>
    ...
    <table class="ui-table ui-table-border ui-table-hover ui-table-striped ui-mt ui-mb">
        ...
        <tbody>
        <?php $array_rs=$this->db->load("select * from sd_admin a left join sd_admin_part b on a.pid=b.id  where 1=1  order by adminid ",0,false,0);$total_rs=count($array_rs);if($total_rs==0){ ?>
        <tr>
...
<?php $i=0;} foreach($array_rs as $rs){ $i++;?>
        ...
        <tr>
            ...
            <td><label class="ui-switch ui-switch-info"><input type="checkbox" <?php if ($rs['islock']==1) { ?> checked<?php }?> data-url="<?php echo U('switchs','type=1&id='.$rs['adminid'].'');?>"><span class="ui-switch-checkbox ui-switch-text"></span></label></td>
            ...
            <td class="ui-text-right">　<a href="javascript:;" data-url="<?php echo U('edit',"id=".$rs['adminid']."");?>" class="edit-iframe"><span class="ui-icon-edit"></span> 编辑</a>　<a href="javascript:;" class="del" data-url="<?php echo U('del','id='.$rs['adminid'].'');?>"><span class="ui-icon-delete"></span> 删除</a></td>
        </tr>
        <?php } if($total_rs>0){ ?>
<?php }?>
        </tbody>
