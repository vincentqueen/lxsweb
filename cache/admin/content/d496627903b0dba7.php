...
<body>
    <div class="position">当前位置：<a href="<?php echo U('index');?>">内容管理</a> > <a href="<?php echo U('recycle');?>">回收站</a></div>
    <div class="border">
        <!---->
        <div class="navbar">
            <div class="lefter">
...
$page=1;}$offset=($page-1)*$pagesize;$way=0;if($offset>1000 && $total_rs>2000 && $offset>$total_rs/2){	$offset=$total_rs-$offset-$pagesize;	$way=1;}if($offset<0){	$pagesize+=$offset;	$offset=0;}$key_="id";$table_="sd_content";$join_="";$where_="where $where";$group_="";$order_="order by ontop desc,ordnum desc,id desc";$field_="*";$keylist=$this->db->getkeylist($key_,$table_,$join_,$where_,$order_,$offset,$pagesize,$way);$array_rs=$this->db->load("select $field_ from $table_ $join_ $keylist $group_ $order_");$pg=new sdcms_page($total_rs,$totalpage,$pagesize,$page);$showpage=$pg->showpage(5);if($total_rs==0){ ?>
            ...
            <tr>
                ...
                <td class="ui-text-left"><?php echo add_city($rs['title'],2);?></td>
                <td><?php echo get_catename($rs['classid']);?></td>
                <td><?php echo $rs['hits'];?></td>
                <td><?php echo iif($rs['ispic']==1,"是","<em>否</em>");?></td>
                <td><?php echo iif($rs['ontop']==1,"是","<em>否</em>");?></td>
                <td><?php echo iif($rs['isnice']==1,"是","<em>否</em>");?></td>
                <td><a href="<?php echo U('edit',"classid=".$rs['classid']."&id=".$rs['id']."");?>"><span class="ui-icon-edit"></span> 编辑</a>　<a href="javascript:;" class="del" data-url="<?php echo U('clear','id='.$rs['id'].'');?>"><span class="ui-icon-delete"></span> 删除</a></td>
            </tr>
            <?php } if($total_rs>0){  }?>
            </tbody>
        </table>
