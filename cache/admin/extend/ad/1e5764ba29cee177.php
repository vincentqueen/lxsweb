...
<body>
    <div class="position">当前位置：扩展管理 > <a href="<?php echo THIS_LOCAL;?>">广告管理</a></div>
    <div class="border">
        <!---->
        <a href="<?php echo U('add');?>" class="ui-btn ui-btn-info">添加广告</a>
        ...
        <table class="ui-table ui-table-border ui-table-hover ui-table-striped ui-mb ui-mt">
            ...
            <tbody>
            <?php $array_rs=$this->db->load("select * from sd_ad  where 1=1  order by ordnum,id ",0,false,0);$total_rs=count($array_rs);if($total_rs==0){ ?>
            <tr>
...
<?php $i=0;} foreach($array_rs as $rs){ $i++;?>
            ...
            <tr>
                ...
                <td><a href="<?php echo U('edit',"id=".$rs['id']."");?>"><span class="ui-icon-edit"></span> 编辑</a><?php if ($rs['akey']=='') { ?>　<a href="javascript:;" class="del" data-url="<?php echo U('del','id='.$rs['id'].'');?>"><span class="ui-icon-delete"></span> 删除</a><?php }?></td>
            </tr>
            <?php } if($total_rs>0){ ?>
<?php }?>
            </tbody>
