...
<div class="ui-tabs ui-tabs-white" data-href="1">
	<ul class="ui-tabs-nav">
	  <li class="active"><a href="<?php echo U('imagelist','type='.$type.'&multiple='.$multiple.'&gid=0&iseditor='.$iseditor.'&islocal='.$islocal.'&thumb='.$thumb.'&water='.$water.'');?>">附件选择</a></li>
	  <li><a href="<?php echo U('imageupload','type='.$type.'&multiple='.$multiple.'&gid='.$gid.'&iseditor='.$iseditor.'&islocal='.$islocal.'&thumb='.$thumb.'&water='.$water.'');?>">本地上传</a></li>
	</ul>
			...
			<div class="ui-row ui-mt-15 ui-mb">
				<div class="filelist-left">
					<ul>
						...
						<?php $array_rs=$this->db->load("select aid,gname,(select count(1) from sd_attachment where gid=a.aid $where_query) as num from sd_attachment_group a  where islock=1  order by ordnum,aid ",0,false,0);$total_rs=count($array_rs);if($total_rs==0){ ?>
<?php } else{ ?>
