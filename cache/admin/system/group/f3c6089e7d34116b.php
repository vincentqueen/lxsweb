...
<body>
    <div class="position">当前位置：系统管理 > <a href="<?php echo THIS_LOCAL;?>">设置分组</a></div>
    <div class="border">
        <!---->
        <a href="javascript:;" data-url="<?php echo U('add');?>" class="add-iframe ui-btn ui-btn-info ui-mr-sm">添加分组</a>
        ...
        <table class="ui-table ui-table-border ui-table-hover ui-table-striped ui-mb ui-mt">
            ...
            <tbody>
            <?php $array_rs=$this->db->load("select * from sd_config_group  where gkey='0'  order by ordnum,id ",0,false,0);$total_rs=count($array_rs);if($total_rs==0){ ?>
<?php } else{ ?>
<?php $i=0;} foreach($array_rs as $rs){ $i++;?>
            <tr>
                ...
                <td class="ui-text-left"><?php echo $rs['gname'];?></td>
                <td><?php if ($rs['types']==1) { ?>网站设置<?php } else { ?>接口设置<?php }?></td>
                ...
                <td><a href="<?php echo U('configgroupfield/index',"gid=".$rs['id']."");?>"><span class="ui-icon-list"></span> 字段管理</a>　<a href="javascript:;" data-url="<?php echo U('edit',"id=".$rs['id']."");?>" class="edit-iframe"><span class="ui-icon-edit"></span> 编辑</a>　<a href="javascript:;" class="del" data-url="<?php echo U('del','id='.$rs['id'].'');?>"><span class="ui-icon-delete"></span> 删除</a></td>
            </tr>
            <?php } if($total_rs>0){ ?>
<?php }?>
            </tbody>
